...
library(MASS)
swsm <- stepAIC(maxlm, direction='backward', scope=list(upper=maxlm, 
																												lower=minlm))
...
<<cache=TRUE>>=
swsm <- stepAIC(maxlm, direction='both', scope=list(upper=maxlm, lower=minlm))
...
swsm <- MASS::stepAIC(minlm, direction='forward', scope=list(upper=maxlm, 
																											 lower=minlm))
s <- summary(swsm)
...
\subsection*{Модель без фактора <<tree>> с нулевым свободным членом}
...
@
	...
	Значение коэффициента детерминации высокое. 
